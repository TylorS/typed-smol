...
describe("typed/ui/Select", () => {
  it("renders a hydrated native listbox popover", () => Effect.gen(function* () {
    const state = yield* Select.makeState({ id: "size", value: "small" });
    const markup = yield* renderToHtmlString(html`${Select.Trigger({ state, content: "Small" })}${Select.Content({ state, content: Select.Option({ state, id: "small", value: "small", content: "Small" }) })}`);
