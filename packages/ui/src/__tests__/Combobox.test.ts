...
describe("typed/ui/Combobox", () => {
  it("renders an input related to its hydrated listbox popover", () => Effect.gen(function* () {
    const state = yield* Combobox.makeState({ id: "search" });
    const markup = yield* renderToHtmlString(html`${Combobox.Input({ state })}${Combobox.Popover({ state, content: Combobox.Item({ state, id: "one", value: "One", content: "One" }) })}`);
