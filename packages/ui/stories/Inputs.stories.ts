...
const button = Fx.unwrap(
  Effect.gen(function* () {
    const state = yield* hydrate(
...
const form = Fx.unwrap(
  Effect.gen(function* () {
    ...
    const state = yield* FormComponent.makeState({
      values: { email: "", phone: { area: 555, line: 1234 } },
    });

    return FormComponent.Form({
      state,
      content: html`${FormComponent.Group({
        label: "Contact information",
        content: html`<div>
          ${FormComponent.Label({ for: "email", content: "Email" })}
          ${FormComponent.EmailInput({ state, name: "email", props: { id: "email", required: true } })}
          ${FormComponent.Error({ state, name: "email" })}
        </div>
        <div>
          ${FormComponent.Label({ for: "phone", content: "Phone" })}
          ${FormComponent.MaskedInput({ state, name: "phone", mask: phone, props: { id: "phone" } })}
          ${FormComponent.Error({ state, name: "phone" })}
        </div>`,
      })}
      ${FormComponent.Submit({ content: "Save" })}
      ${FormComponent.Reset({ state, content: "Reset" })}`,
    });
  }),
);
