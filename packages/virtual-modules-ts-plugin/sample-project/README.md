...
1. **`virtual:foo`** – Simple custom plugin (`plugin.cjs`) that exports a typed `Foo` interface and `DEFAULT`.
...
- **`plugin.cjs`** – Implements the `virtual:foo` virtual module (used by both the TS plugin and `vmc`).
- **`plugins/router-plugin.cjs`** – Bundled CJS build of `createRouterVirtualModulePlugin()` from `@typed/app`. Generated by `pnpm build:plugins`.
...
- **Custom prefix/name** – To test other options, add a second bundle (e.g. `scripts/router-plugin-custom-entry.mjs` calling `createRouterVirtualModulePlugin({ prefix: "app-routes:", name: "app-routes" })`) and build to `plugins/router-plugin-custom.cjs`, then add that plugin to `tsconfig.json` and import `app-routes:./routes` where needed.
...
# Type-check with virtual modules (vmc uses vmc.config.js; resolves virtual:foo only)
pnpm run vmc --noEmit
```
