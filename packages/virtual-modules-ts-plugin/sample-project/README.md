...
- **`api/`** – Top-level HttpApi endpoint modules. Each file exports `route`, `method`, optional `success`/`error` schemas, and `handler`. Use **`defineApiHandler(route, method, { success, error })(handler)`** from `@typed/app` so handler params and return types are inferred from the route and schemas (see `api/status.ts`).
...
- **Type-level helpers (from `@typed/app`)**:
  - **API**: `defineApiHandler(route, method, schemas?)(handler)` — infers `path`/`query`/`headers`/`body` and success/error from route and schemas; use in `api/` endpoint files (see `api/status.ts`).
