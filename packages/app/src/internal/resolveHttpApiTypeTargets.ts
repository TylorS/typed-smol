...
/**
 * Type specs for HttpApi virtual module structural validation.
 * Resolved from program imports; used with createTypeInfoApiSession typeTargets.
 * Module paths align with Effect package exports (effect, effect/Schema, effect subpaths).
 */
const HTTPAPI_TYPE_SPECS: readonly { id: string; module: string; exportName: string }[] = [
...
/**
 * Resolve well-known HttpApi-related types from the program for structural assignability checks.
 * Scans source files for imports from the given modules and extracts the type of each export.
 * Use with createTypeInfoApiSession typeTargets option.
 *
 * @returns Array of ResolvedTypeTarget; entries may be missing if the program
 *   does not import from the required modules.
 */
export function resolveHttpApiTypeTargets(
  program: ts.Program,
  tsMod: TS,
): ResolvedTypeTarget[] {
  ...
  for (const sourceFile of program.getSourceFiles()) {
    ...
    tsMod.forEachChild(sourceFile, (node) => {
      ...
      const addNamed = (name: ts.ImportSpecifier) => {
        ...
        const spec = HTTPAPI_TYPE_SPECS.find(
      ...
      if (binding.namedBindings) {
        ...
        const spec = HTTPAPI_TYPE_SPECS.find((s) => s.module === moduleSpec);
        if (!spec || found.has(spec.id)) return;
  ...
  return HTTPAPI_TYPE_SPECS.filter((s) => found.has(s.id)).map((s) => ({
    id: s.id,
    type: found.get(s.id)!,
  }));
}
