...
- Remaining-gap closure pass added the host-first substrate (`Dom.HostOptions`, event chaining, ref composition), host overrides for Button/Dialog/Popover/Select, renderable Popover anchor-positioning values, Select typeahead and hidden input form participation, schema-backed Form validation/reset plus array-only Push/Remove, rendered browser coverage for Popover/Dialog/Combobox/Select, and native dialog invoker capture/focus restoration.
...
- Prerelease cleanup pass removed the `@typed/ui` modal focus shim and overlay mounting shim, removed VS Code tsconfig plugin-list fallback loading in favor of `vmc.config.ts`, and rejected virtual preview artifact paths outside `node_modules/.typed/virtual`.
- Prerelease cleanup focused verification: RED `pnpm --filter @typed/ui exec vitest run src/AriakitParity.test.ts` and `pnpm --filter @typed/virtual-modules-vscode exec vitest run src/resolver.test.ts src/virtualPreviewDisk.test.ts` failed on the removed-shim/fallback expectations; GREEN `pnpm --filter @typed/ui test`, `pnpm --filter @typed/virtual-modules-vscode test`, `pnpm --filter @typed/compiler test -- diagnostics`, and `pnpm --filter @typed/app test -- RouterVirtualModulePlugin HttpApiVirtualModulePlugin` passed.
- Composite DRY pass centralized active-id movement in `Composite.moveActiveId`/`moveActiveItem` and removed local `nextActiveId` implementations from Toolbar, Tabs, Listbox, Menu, Menubar, Select, Combobox, and RadioGroup. Direct execution was used because the available subagent tool only permits delegation when the user explicitly asks for subagents.
- Composite DRY focused verification: RED `pnpm --filter @typed/ui exec vitest run src/Composite.test.ts` failed because `moveActiveId`/`moveActiveItem` were missing; GREEN focused component runs passed for Composite/Toolbar/Tabs/Listbox, Menu/Menubar, Select/Combobox, and RadioGroup.
- Composite DRY full verification: `pnpm --filter @typed/ui test` passed with 25 files and 127 tests; `pnpm --filter @typed/ui build` passed; `pnpm --filter @typed/ui exec vitest typecheck --run src/ComponentOptions.test.ts` passed with 1 file and 5 type-test cases; `pnpm --filter @typed/ui run test:browser` passed with 2 files and 6 tests.
