...
- Finalization on `codex/typed-beta` required rebuilding the local `better-sqlite3` native binding with `npm run build-release` in `node_modules/.pnpm/better-sqlite3@12.10.0/node_modules/better-sqlite3` before RealWorld repository/API/SSR tests could execute on Node 24.16.0.
- RealWorld presentation tests directly import `happy-dom`; keep `happy-dom: catalog:` in `examples/realworld` dev dependencies and in the package dependency guard.
- Local acceptance package tests should not require `.temp/references/realworld` to exist during normal unit tests. They should prove wrapper scripts reference the external checkout and that specs are not vendored; wrapper commands own prerequisite checks.
- Final verification on 2026-05-22 passed with `pnpm --filter typed-realworld typecheck`, `pnpm --filter typed-realworld test`, `pnpm --filter typed-realworld build`, `pnpm --filter @typed/ui test`, `pnpm --filter @typed/ui build`, focused `@typed/app` router/httpapi/browser/server virtual-module tests, and `git diff --check`.
- `Route.NonNegativeInt(...)` and `Route.PositiveInt(...)` are first-party router primitives for numeric string route/query params. Prefer these over spelling `Route.ParamWithSchema("limit", Schema.NumberFromString.pipe(...)).optional()` in app/example modules.
- RealWorld route query fragments live in `src/common/routeParams.ts`, not `src/domain`, so shared page/limit/offset route declarations do not violate the domain rule forbidding `@typed/*` imports.
- Task 29 verification passed with `pnpm --filter @typed/router exec vitest run src/Route.test.ts -t "non-negative integer|positive integer"`, `pnpm --filter @typed/router exec vitest run src/Route.test.ts`, `pnpm --filter @typed/router build`, `pnpm --filter @typed/router test`, `pnpm --filter typed-realworld typecheck`, `pnpm --filter typed-realworld exec vitest run src/tests/api/api.test.ts`, `pnpm --filter typed-realworld build`, `pnpm build`, and scoped `git diff --check`.
