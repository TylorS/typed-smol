...
- Route `Handler` accepts plain values, `Effect`, `Stream`, `Fx`, decoded-param functions returning any of those, and `RefSubject<Params>` functions returning any of those. Because the alias is intentionally broad, route modules that need typed RefSubject operations should annotate the parameter with `RefSubject.RefSubject<Params>`.
