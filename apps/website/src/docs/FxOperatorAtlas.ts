...
export const renderFxOperatorAtlasMarkdown = (): string => {
  ...
  return [
    ...
    "Read each figure as one concrete execution, not a promise of exact scheduler interleaving. A dot is an idle slot, ^ starts work, | marks a run returning, ! labels a delivered Cause, and x marks interruption. Timed figures state their slot duration; other slots show causal order. Service, state, callback, and consumer lanes describe control activity rather than additional Fx values. Effect callback examples use serialized input unless their caption explicitly describes concurrency.",
