...
const sourceLabel = (evidence: UiAccessibilityEvidence): string =>
  `${evidence.file}: ${evidence.detail}`;

const familyReferenceHref = (family: string): string =>
  `/reference/modules/${encodeURIComponent(`@typed/ui/${family}`)}`;

const evidenceKind = (evidence: UiAccessibilityEvidence): string => {
  if (evidence.file.includes(".browser.test.")) return "browser";
  if (evidence.file.includes("/__tests__/")) return "contract";
  return "source";
};

/** Renders the source-backed catalog embedded by the choosing-components Explore guide. */
export const UiAccessibilityCatalog = () =>
  html`<section class="ui-accessibility-catalog" aria-labelledby="ui-accessibility-catalog-title">
    <header class="ui-accessibility-catalog__intro">
      <span class="index">37 PUBLIC FAMILIES / SOURCE-BACKED</span>
      <h2 id="ui-accessibility-catalog-title">Accessibility contract catalog</h2>
      <p>
        Every card separates behavior supplied by Typed from decisions that remain with the
        application author. Family names open the corresponding API module; standards links open the
        platform contract the implementation follows.
      </p>
      <div class="ui-accessibility-catalog__bounds">
        <section>
          <h3>What Typed can verify</h3>
          <p>
            Types verify required inputs. Render tests verify hosts, roles, states, and
            relationships. Browser tests verify the keyboard, focus, event, and native behavior
            listed by each family.
          </p>
        </section>
        <section>
          <h3>What only an application can verify</h3>
          <p>
            Meaningful names and content, reading order, task fit, announcement quality, and
            contrast after application styling still require review with real content and assistive
            technology.
          </p>
        </section>
      </div>
    </header>
    <div class="ui-accessibility-catalog__grid">
      ${uiAccessibilityFamilies.map(
        (entry) => html`<section
          class="ui-accessibility-card"
          aria-labelledby="ui-family-${entry.id}"
        >
          <header>
            <h3 id="ui-family-${entry.id}">${entry.label}</h3>
            <nav class="ui-accessibility-card__families" aria-label="${entry.label} API modules">
              ${entry.families.map(
                (family) => html`<a href=${familyReferenceHref(family)}>${family}</a>`,
              )}
            </nav>
          </header>
          <div class="ui-accessibility-card__contracts">
            <section>
              <h4>Typed verifies</h4>
              <ul>
                ${entry.typedVerifies.map((claim) => html`<li>${claim}</li>`)}
              </ul>
            </section>
            <section>
              <h4>Authors must provide</h4>
              <ul>
                ${entry.authorsMustProvide.map((requirement) => html`<li>${requirement}</li>`)}
              </ul>
            </section>
          </div>
          <nav class="ui-accessibility-card__standards" aria-label="${entry.label} standards">
            ${entry.references.map(
              (reference) => html`<a href=${reference.href} rel="external">${reference.label}</a>`,
            )}
          </nav>
          <details class="ui-accessibility-card__evidence">
            <summary>Implementation and test evidence</summary>
            <ul>
              ${entry.evidence.map(
                (item) => html`<li>
                  <span>${evidenceKind(item)}</span><code>${sourceLabel(item)}</code>
                </li>`,
              )}
            </ul>
          </details>
