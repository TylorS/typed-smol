---
name: effect-facet-unstable-http-router
description: Guidance for facet `effect/unstable/http#router` focused on APIs like Template, FindMyWay, and HttpMethod. Load after `effect-skill-router` when this facet is the primary owner.
---

# Effect Facet unstable/http#router

## Owned scope

- Owns only `effect/unstable/http#router`.
- Parent module: `effect/unstable/http`.

## What it is for

- routing and method template surfaces. Module-specific APIs and usage patterns for Effect programs.
...
- Keep work focused on the `router` concern for `effect/unstable/http`.
