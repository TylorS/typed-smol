---
name: effect-facet-unstable-cluster-core
description: Guidance for facet `effect/unstable/cluster#core` focused on APIs like Runner, Runners, and RunnerHealth. Load after `effect-skill-router` when this facet is the primary owner.
---

# Effect Facet unstable/cluster#core

## Owned scope

- Owns only `effect/unstable/cluster#core`.
- Parent module: `effect/unstable/cluster`.

## What it is for

- entity/shard core domain. Module-specific APIs and usage patterns for Effect programs.
...
- Keep work focused on the `core` concern for `effect/unstable/cluster`.
