---
name: effect-facet-unstable-cluster-storage
description: Guidance for facet `effect/unstable/cluster#storage` focused on APIs like RunnerStorage, MessageStorage, and SqlMessageStorage. Load after `effect-skill-router` when this facet is the primary owner.
---

# Effect Facet unstable/cluster#storage

## Owned scope

- Owns only `effect/unstable/cluster#storage`.
- Parent module: `effect/unstable/cluster`.

## What it is for

- message/runner storage backends. Module-specific APIs and usage patterns for Effect programs.
...
- Keep work focused on the `storage` concern for `effect/unstable/cluster`.
