---
name: effect-facet-unstable-http-httpclienterror
description: Guidance for facet `effect/unstable/http/HttpClientError` focused on APIs like DecodeError, EncodeError, and EmptyBodyError. Load after `effect-skill-router` when this facet is the primary owner.
---

# Effect Facet unstable/http/HttpClientError

## Owned scope

- Owns only `effect/unstable/http/HttpClientError`.
- Parent module: `effect/unstable/http`.
- Source anchor: `packages/effect/src/unstable/http/HttpClientError.ts`.
...
- Facet source: `packages/effect/src/unstable/http/HttpClientError.ts`
